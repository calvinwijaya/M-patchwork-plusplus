...
// Add necessary headers for LAS file handling
...
int filename_length = std::string("demo_visualize.cpp").length();
...
void read_pcd(const std::string& pcd_path, Eigen::MatrixXf& cloud) {
    ...
    // Convert Open3D PointCloud to Eigen Matrix
    cloud.resize(pcd->points_.size(), 3);
    for (size_t i = 0; i < pcd->points_.size(); ++i) {
        cloud.row(i) << pcd->points_[i].x(), pcd->points_[i].y(), pcd->points_[i].z();
    }
}
...
int main(int argc, char* argv[]) {
  ...
  cout << "Original Points  #: " << cloud.rows() << endl;
  ...
  visualizer.CreateVisualizerWindow("Open3D", 1366, 768);
  ...
  // Save ground point cloud to a new file
  std::shared_ptr<geometry::PointCloud> geo_ground_save(new geometry::PointCloud);
  eigen2geo(ground, geo_ground);

  // Save ground point cloud to a new file
  open3d::io::WritePointCloud("ground.pcd", *geo_ground, open3d::io::WritePointCloudOption(false, false));
